...
public final class CheckoutController {
    ...
    @FXML private TableView<CartItem> tbl;
    @FXML private TableColumn<CartItem,String>  colEvent;
    @FXML private TableColumn<CartItem,String>  colCat;
    ...
    public void init(String user) {
        ...
        colEvent.setCellValueFactory(c ->
                new SimpleStringProperty(c.getValue().event().getName()));
        colCat.setCellValueFactory(c ->
                new SimpleStringProperty(c.getValue().category().getCategoryName()));   // ← fixed
        colQty.setCellValueFactory(c ->
                new SimpleIntegerProperty(c.getValue().quantity()).asObject());
        colPrice.setCellValueFactory(c ->
                new SimpleDoubleProperty(c.getValue().category().getPrice()).asObject()); // ← fixed
        colTotal.setCellValueFactory(c ->
                new SimpleDoubleProperty(c.getValue().totalPrice()).asObject());
    ...
    private void confirm() {
        Cart.get().items().forEach(i ->
                ServiceLocator.SERVICE.reserve(
                        currentUser,
                        i.event(),
                        i.category().getCategoryName(),     // ← fixed
                        i.quantity()));
